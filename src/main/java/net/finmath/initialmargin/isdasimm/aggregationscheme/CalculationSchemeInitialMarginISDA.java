...
public class CalculationSchemeInitialMarginISDA {
	...
	public class ParameterCollection {
		public ParameterCollection() {//hard values inserted by Mario Viehmann
			...
			Double[][] value1 = new Double[1][riskWeightsRegularCurrency.length];
	...
	//                        .flatMap(
	//                                key -> tradeSensitivityMap.get(key).getKeySet().stream().filter(k -> k!=null &&  k.getRiskClass().equals(riskClass) && k.getRiskType().equals(riskTypeString) && k.getSimmBucket().equals(bucketKey)).map(k -> k.getQualifier())
